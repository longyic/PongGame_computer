...
void drawline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,uint8_t color) {
	int8_t dx, dy, d, x, y, slope ;
	slope = abs(y1 - y0) > abs(x1 - x0);
	if (slope)                      //if slope>1 change x and y
	{
		swap(x0, y0);
		swap(x1, y1);
	}
	if (x0 > x1)                    //if number from big to small, change x1 x2
	{
		swap(x0, x1);
		swap(y0, y1);
	}
...
void fillrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	for (uint8_t a = x; a < x+w; a++) {
		for (uint8_t b = y; b < y+h; b++) {
			setpixel(buff, a, b, color);
		}
	}
}

void clearrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	for (uint8_t a = x; a < x+w; a++) {
		for (uint8_t b = y; b < y+h; b++) {
			clearpixel(buff, a, b);
		}
...
void drawrect(uint8_t *buff,uint8_t x, uint8_t y, uint8_t w, uint8_t h,uint8_t color) {
	for (uint8_t a = x; a < x+w; a++) {
		setpixel(buff, a, y, color);
		setpixel(buff, a, y+h-1, color);
	}
	for (uint8_t b = y; b < y+h; b++) {
		setpixel(buff, x, b, color);
		setpixel(buff, x+w-1, b, color);
	}
}
